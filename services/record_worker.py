    ...
    def run(self):
        ...
        while self.running:
            ...
            if not snapshot.get("recording") or not snapshot.get("order_code"):
                ...
                continue
            # ép phải có nhân viên mới được ghi hình
            #if not snapshot.get("employee_id"):
            #    self.state.set_error(cam_id, "Missing employee for recording")
            #    self._close_session("STOP")
            ...
            if self.writer is None or self.capture is None:
                if not self._open_session(snapshot):
                    time.sleep(1)
                    continue
            elif self.current_employee != snapshot.get("employee_id", ""):
